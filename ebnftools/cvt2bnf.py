    ...
    for k in orig_as_dict:
        if k not in new_as_dict:
            ...
            if isfinite(orig_as_dict, orig_as_dict[k]) and isfinite(new_as_dict, new_as_dict[k]):
                ...
                if co != cn:
                    print(f"ERROR: Count for rule {k}: EBNF count {co} != BNF count {cn}")
                else:
                    print(f"INFO: {k} is okay [{cn} == {co}]")
                ...
                if paranoid:
                    ...
                    if s1 != s2:
                        print(f"ERROR: Generated strings for {k} don't match: {s1 - s2} and {s2 - s1}")
                    else:
                        print(f"INFO: Generated strings for {k} match")
            else:
                print(f"ERROR: Can't check {k} for weak equivalence, contains infinite productions")
